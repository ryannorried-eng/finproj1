...
def submit_bet(bet, store) -> str:
    ...
    log.info("submit_bet persisted bet_id=%s legs=%s stake=%s", bet_id, legs_count, stake)
    return bet_id
...
def settle_bet(bet_id: str, outcome: str, store) -> None:
    ...
    log = get_logger(__name__, source="bet_service")
    before = getattr(getattr(store, "_conn", None), "total_changes", None)
    close_bet_clv(bet_id, store)
    settle_bet_persistent(bet_id, outcome, store)
    after = getattr(getattr(store, "_conn", None), "total_changes", None)
    changed = (after - before) if (before is not None and after is not None) else "unknown"
    log.info("settle_bet persisted bet_id=%s outcome=%s rows_updated=%s", bet_id, outcome, changed)
